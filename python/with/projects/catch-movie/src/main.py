...
def main():
    ...
    title = customtkinter.CTkLabel(app, text="HDFilmCehennemi Embed Finder", font=("system", 20))
    title.grid(row=0, column=0, padx=10, pady=10, sticky="nsew")
    
    description = customtkinter.CTkLabel(app, text="Enter your hdfilmcehennemi url here and click 'Find' button.")
    description.grid(row=1, column=0, padx=10, pady=10, sticky="nsew")
    
    entry = customtkinter.CTkEntry(app, placeholder_text="https://www.hdfilmcehennemi.nl/1-the-hobbit-smaugun-corak-topraklari-hdf-5/")
    entry.grid(row=2, column=0, padx=10, pady=10, sticky="ew")
    
    # ✅ Pass a lambda to avoid executing the function immediately
    button = customtkinter.CTkButton(app, text="Find", command=lambda: open_in_browser(entry))
